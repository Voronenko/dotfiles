...
install-console-logreader-lnav:
	curl -sLo /tmp/lnav.zip https://github.com/tstack/lnav/releases/download/v0.12.2/lnav-0.12.2-linux-musl-x86_64.zip
	cd /tmp && unzip lnav.zip
	mv /tmp/lnav-0.12.2/lnav ~/dotfiles/bin
	lnav -i extra
...
install-console-wtfutil:
	curl -sLo /tmp/wtf.tar.gz  https://github.com/wtfutil/wtf/releases/download/v0.43.0/wtf_0.43.0_linux_amd64.tar.gz
	tar -xvzf /tmp/wtf.tar.gz -C /tmp
	cp /tmp/wtf_0.43.0_linux_amd64/wtfutil ~/dotfiles/bin/wtfutil
...
install-k8s-k9s:
	mkdir -p /tmp/k9s
	curl -sLo /tmp/k9s/k9s.tar.gz https://github.com/derailed/k9s/releases/download/$(shell curl -s https://api.github.com/repos/derailed/k9s/releases/latest | grep tag_name | cut -d '"' -f 4)/k9s_Linux_amd64.tar.gz
